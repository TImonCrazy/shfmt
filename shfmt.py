...
class Formatter(object):
  ...
  Wrapper for the process used to format the Terraform file.
	...
	def format(self, region):
		'''
		Attempts to format the code, raising any errors in an output panel if
		they arise.

		:param sublime.Region region: The region for the file to format
		:returns: str. Returns the formatted content if no errors arose, else the
		original content.
		'''
...
class shfmtCommand(sublime_plugin.TextCommand):
	'''
	The `shfmt` command, invoked by the command palette or before a file save.
	'''
	def is_enabled(self):
		'''
		Checks if the current file viewed is a Bash file (.sh) or not.
...
class shfmtListener(sublime_plugin.EventListener):
	def on_pre_save(self, view):
		'''
		Called before a dirty view is stored to disk, running the `shfmt` command.
		'''
